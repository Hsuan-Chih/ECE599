...
# timings.shape
plaintext = np.array(timings[:, range(16)], dtype=np.uint8)
times = np.array(timings[:, -1], dtype=np.uint16)  # (1000000, )
# np.shape(plaintext)
guess_key = np.arange(256).reshape(1, -1)  # (1, 256)
...
def process_key(position):
    ...
    for k in range(256):  # guess our key
        # group = [[], []]
        np_first_hex = np.bitwise_and(lut_sbx_input[:, k], 0xf0) >> 4
        np_second_hex = np.bitwise_and(lut_sbx_input[:, k], 0x0f)
    ...
    # print(position)
    # return result.append(np.argmax(record_in_loop))
    return np.argmax(record_in_loop)


if __name__ == "__main__":
    with mp.Pool(processes=4) as pool:
        # Use the map method to apply the function to each position in parallel
        result = pool.map(process_key, range(16))

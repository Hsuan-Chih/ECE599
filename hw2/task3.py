...
# print(plaintexts[:, 0])
# print(np.reshape(plaintexts[:, 0], (-1, 1)))    # (10000, 1)
...
def process_key(position):
    lut_sbx_input = np.bitwise_xor(np.reshape(plaintexts[:, position], (-1, 1)), guess_key)  # (10000, 256)
    # print(lut_sbx_input[:, 255].shape)
    iv = np.empty((0, 256), dtype=np.uint8)
    for n in range(10000):  # number of traces
        ...
        iv = np.append(iv, [sbx_output], axis=0)  # (10000, 256)

    # apply hamming weight on iv
    hm = np.vectorize(hamming_weight)(iv)  # (10000, 256)

    # stochastic analysis
    max_r = []
    for k in range(256):  # rows correspond to key hypothesis
        r = []
        for t in range(50):  # columns correspond to data points (time axis)

import numpy as np
import hashlib
import time as t

start_time = t.time()

simple_ctxt1 = [174, 44, 204, 43, 18, 196, 238, 88, 3, 227, 92, 0, 137, 106, 205, 88]
simple_ftxt1 = [128, 44, 204, 43, 18, 196, 238, 171, 3, 227, 159, 0, 137, 186, 205, 88]

simple_ctxt2 = [41, 4, 148, 29, 23, 74, 41, 127, 125, 148, 36, 219, 29, 127, 4, 58]
simple_ftxt2 = [186, 4, 148, 29, 23, 74, 41, 160, 125, 148, 59, 219, 29, 172, 4, 58]

# Load ctext/ftext pairs in the correct AES column order
simple_ctxt1 = np.reshape(simple_ctxt1, (4, 4), order='F').astype(np.uint8)
simple_ftxt1 = np.reshape(simple_ftxt1, (4, 4), order='F').astype(np.uint8)

simple_ctxt2 = np.reshape(simple_ctxt2, (4, 4), order='F').astype(np.uint8)
simple_ftxt2 = np.reshape(simple_ftxt2, (4, 4), order='F').astype(np.uint8)

# all variables we need in "preliminary filtering"
c0 = simple_ctxt1[np.unravel_index(0, shape=(4, 4), order='F')]
cp0 = simple_ftxt1[np.unravel_index(0, shape=(4, 4), order='F')]
c13 = simple_ctxt1[np.unravel_index(13, shape=(4, 4), order='F')]
cp13 = simple_ftxt1[np.unravel_index(13, shape=(4, 4), order='F')]
c10 = simple_ctxt1[np.unravel_index(10, shape=(4, 4), order='F')]
cp10 = simple_ftxt1[np.unravel_index(10, shape=(4, 4), order='F')]
c7 = simple_ctxt1[np.unravel_index(7, shape=(4, 4), order='F')]
cp7 = simple_ftxt1[np.unravel_index(7, shape=(4, 4), order='F')]

# all variables we need in "finding final candidate"
cs0 = simple_ctxt2[np.unravel_index(0, shape=(4, 4), order='F')]
cps0 = simple_ftxt2[np.unravel_index(0, shape=(4, 4), order='F')]
cs13 = simple_ctxt2[np.unravel_index(13, shape=(4, 4), order='F')]
cps13 = simple_ftxt2[np.unravel_index(13, shape=(4, 4), order='F')]
cs10 = simple_ctxt2[np.unravel_index(10, shape=(4, 4), order='F')]
cps10 = simple_ftxt2[np.unravel_index(10, shape=(4, 4), order='F')]
cs7 = simple_ctxt2[np.unravel_index(7, shape=(4, 4), order='F')]
cps7 = simple_ftxt2[np.unravel_index(7, shape=(4, 4), order='F')]


SBOX = np.array([
        
], dtype=np.uint8)

# Inverse of AES SBOX: give you the hex to indicate the corresponding row and column
ISBOX = SBOX.argsort()

# AES MixCols matrix
MIXCOLS = np.array([[2, 3, 1, 1],
                    [1, 2, 3, 1],
                    [1, 1, 2, 3],
                    [3, 1, 1, 2]])


# Galois multiplication by 2 (for MixCols)
def galois_mult_2(a):
    temp = (a << 1) & 0xff

    if (a & 0x80):
        temp ^= 0x1b

    return temp


# Galois multiplication by 3 (for MixCols)
def galois_mult_3(a):
    return galois_mult_2(a) ^ a


D = []
mixcol = MIXCOLS[:, 0]
for x in range(1, 255+1):
    D_element = []
    for j in range(4):
        out = None
        if mixcol[j] == 1:
            out = x
        if mixcol[j] == 2:
            out = galois_mult_2(x)
        if mixcol[j] == 3:
            out = galois_mult_3(x)
        D_element.append(out)
    D.append(D_element)

# print("Length of lookup table:", len(D))    # Length of lookup table: 255
# print(D[0][0])  # D[x=0, 0]   x: 0 ~ 254
d0 = np.array(D)[:, 0]  # (255, )
d1 = np.array(D)[:, 1]  # (255, )
d2 = np.array(D)[:, 2]  # (255, )
d3 = np.array(D)[:, 3]  # (255, )

# used for comparing the candidates to each group
D_f01 = np.dstack((d0, d1)).reshape(255, 2)
D_f02 = np.dstack((d0, d1, d2)).reshape(255, 3)
D_f03 = np.dstack((d0, d1, d2, d3)).reshape(255, 4)


def check_keybytes(k_0: int, k_13: int, k_10: int, k_7: int):
    keybytes = bytes([k_0, k_13, k_10, k_7])
    hasher = hashlib.sha3_256()
    hasher.update(keybytes)
    key_hash = hasher.hexdigest()
    if key_hash == '4409976e63e88e6d0ef93405e6b6d678c2a498d22dcaa72b28c8c9cd6233ec7f':
        print("Congratulations! Correct 4 keybytes found")
        return True

    print("Not quite right")
    return False


if __name__ == '__main__':
    # first for-loop
    print("Executing Kc2 ...")
    Kc2 = []
    for k0 in range(256):           # 256
        for k13 in range(256):      # 256
            result_0 = np.bitwise_xor(ISBOX[np.bitwise_xor(k0, c0)], ISBOX[np.bitwise_xor(k0, cp0)])
            result_13 = np.bitwise_xor(ISBOX[np.bitwise_xor(k13, c13)], ISBOX[np.bitwise_xor(k13, cp13)])

            D_comp01 = np.array([result_0, result_13])
            if any(np.array_equal(D_comp01, x) for x in D_f01):
                Kc2.append((k0, k13))

            # if np.any(result_0 == d0) and np.any(result_13 == d1):
            # if np.all([np.any(result_0 == d0), np.any(result_13 == d1)]):
            #     Kc2.append((k0, k13))
    # print(len(Kc2))     # 252

    # second for-loop
    print("Executing Kc3 ...")
    Kc3 = []
    for k10 in range(256):          # 256
        for k0, k13 in Kc2:         # 252
            result_0 = np.bitwise_xor(ISBOX[np.bitwise_xor(k0, c0)], ISBOX[np.bitwise_xor(k0, cp0)])
            result_13 = np.bitwise_xor(ISBOX[np.bitwise_xor(k13, c13)], ISBOX[np.bitwise_xor(k13, cp13)])
            result_10 = np.bitwise_xor(ISBOX[np.bitwise_xor(k10, c10)], ISBOX[np.bitwise_xor(k10, cp10)])

            D_comp02 = np.array([result_0, result_13, result_10])
            if any(np.array_equal(D_comp02, x) for x in D_f02):
                Kc3.append((k0, k13, k10))

            # if np.any(result_0 == d0) and np.any(result_13 == d1) and np.any(result_10 == d2):
            # if np.all([np.any(result_0 == d0), np.any(result_13 == d1), np.any(result_10 == d2)]):
            #     Kc3.append((k0, k13, k10))
    # print(len(Kc3))     # 248

    # third for-loop
    print("Executing Kc4 ...")
    Kc4 = []
    for k7 in range(256):           # 256
        for k0, k13, k10 in Kc3:    # 248
            result_0 = np.bitwise_xor(ISBOX[np.bitwise_xor(k0, c0)], ISBOX[np.bitwise_xor(k0, cp0)])
            result_13 = np.bitwise_xor(ISBOX[np.bitwise_xor(k13, c13)], ISBOX[np.bitwise_xor(k13, cp13)])
            result_10 = np.bitwise_xor(ISBOX[np.bitwise_xor(k10, c10)], ISBOX[np.bitwise_xor(k10, cp10)])
            result_7 = np.bitwise_xor(ISBOX[np.bitwise_xor(k7, c7)], ISBOX[np.bitwise_xor(k7, cp7)])

            D_comp03 = np.array([result_0, result_13, result_10, result_7])
            if any(np.array_equal(D_comp03, x) for x in D_f03):
                Kc4.append((k0, k13, k10, k7))

            # if np.any(result_0 == d0) and np.any(result_13 == d1) and np.any(result_10 == d2) and np.any(result_7 == d3):
            # if np.all([np.any(result_0 == d0), np.any(result_13 == d1), np.any(result_10 == d2), np.any(result_7 == d3)]):
            #     Kc4.append((k0, k13, k10, k7))
    # print(len(Kc4))     # 240

    # fourth for-loop: second pair
    print("Executing final_result ...")
    final_result = []
    for k0, k13, k10, k7 in Kc4:    # 240
        result_0 = np.bitwise_xor(ISBOX[np.bitwise_xor(k0, cs0)], ISBOX[np.bitwise_xor(k0, cps0)])
        result_13 = np.bitwise_xor(ISBOX[np.bitwise_xor(k13, cs13)], ISBOX[np.bitwise_xor(k13, cps13)])
        result_10 = np.bitwise_xor(ISBOX[np.bitwise_xor(k10, cs10)], ISBOX[np.bitwise_xor(k10, cps10)])
        result_7 = np.bitwise_xor(ISBOX[np.bitwise_xor(k7, cs7)], ISBOX[np.bitwise_xor(k7, cps7)])

        D_comp_final = np.array([result_0, result_13, result_10, result_7])
        if any(np.array_equal(D_comp_final, x) for x in D_f03):
            final_result.append((k0, k13, k10, k7))

        # if np.any(result_0 == d0) and np.any(result_13 == d1) and np.any(result_10 == d2) and np.any(result_7 == d3):
        # if np.all([np.any(result_0 == d0), np.any(result_13 == d1), np.any(result_10 == d2), np.any(result_7 == d3)]):
        #     final_result.append((k0, k13, k10, k7))
    print(final_result)
    print("Length of final result: ", len(final_result))

    # final result: [(168, 138, 164, 45)]
    check_keybytes(final_result[0][0], final_result[0][1], final_result[0][2], final_result[0][3])

end_time = t.time()
print("Time: ", end_time - start_time)



